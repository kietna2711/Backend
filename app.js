...
cron.schedule("*/1 * * * *", async () => {
  ...
  const eligibleVouchers = vouchers.filter(
    (v) => v.startDate <= nowVN && v.endDate >= nowVN
  );
  console.log(
    "Voucher đủ điều kiện kích hoạt (giờ VN):",
    eligibleVouchers.map((v) => v.discountCode)
  );
  try {
    ...
    if (result.modifiedCount > 0) {
      console.log(
        `[CRON] Đã tự động kích hoạt ${result.modifiedCount} voucher đến ngày bắt đầu.`
      );
    }
  } catch (err) {
...
cron.schedule("*/1 * * * *", async () => {
  ...
  try {
    ...
    if (result.modifiedCount > 0) {
      console.log(
        `[CRON] Đã tự động tắt ${result.modifiedCount} voucher hết hạn.`
      );
    }
  } catch (err) {
